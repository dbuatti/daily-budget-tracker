...
const LogTransaction = () => {
  // All hooks must be called at the top level - no conditionals before this
  const {
    modules,
    gearTravelFund,
    totalSpent,
  ...
  // Early returns are OK after hooks are called
  if (isLoading) {
    return (
  ...
  const visibleModules = modules.filter(
    (module) => module.categories.some((category) => category.tokens.length > 0)
  );
