-- Create the table to store the weekly budget state for each user
...
CREATE POLICY "Users can only see their own budget state" ON public.weekly_budget_state
FOR SELECT TO authenticated USING (auth.uid() = user_id);

-- Policy: Users can insert their own budget state (on first creation)
