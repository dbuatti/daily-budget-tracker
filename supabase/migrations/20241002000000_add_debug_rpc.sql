-- This migration contains the corrected RPC functions for daily spend calculation
...
BEGIN
  ...
  -- Determine target date based on rollover hour
  ...
  RAISE NOTICE '[get_daily_spent_amount] User: %, TZ: %, Rollover: %, Current TZ Time: %, Target Date: %, Start: %, End: %, Spent: %', 
    p_user_id, v_timezone, v_rollover_hour, v_current_time_in_tz, v_target_date, v_boundaries.start_time, v_boundaries.end_time, v_total_spent;

  RETURN v_total_spent;
END;
$function$;

-- Also update the debug function to use the same logic
